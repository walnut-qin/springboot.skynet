...
class CoreService {
     ...
     */
    @Autowired
    AgeDurationToStringConverter ageDurationToStringConverter;
    ...
    private StateResult getStateCore(EscortMainInfo escortMainInfo) {
        ...
        if (stateRecs != null && !stateRecs.isEmpty() && stateRecs.get(0).getState() == StateEnum.生效中) {
            offset = 14;
        } else {
            offset = 2;
        }
        ...
        var nats = natsConverter.convert(NatsConverter.Key.builder()
                ...
                .duration(Duration.ofDays(offset))
                .build());
        if (nats != null) {
            if (nats.getNegative()) {
                return StateResult.builder()
        ...
        if (feeDetails != null && !feeDetails.isEmpty()) {
            ...
            return StateResult.builder()
                    .state(StateEnum.等待院内核酸检测结果)
                    .reason("无核酸检测结果, 14天内开立过核酸项目").build();
        }

        // 判断是否有未审核的院外结果
        List<EscortAnnexInfo> annexInfos = annexInfoMapper.queryAnnexInfos(EscortAnnexInfoMapper.Key.builder()
                .cardNo(helper.getCardNo())
                .checked(false)
                .build());
        if (annexInfos != null && !annexInfos.isEmpty()) {
            return StateResult.builder()
                    .state(StateEnum.等待院外核酸检测结果审核)
                    .reason("无核酸检测结果，已上传院外结果").build();
        }

        return StateResult.builder()
                .state(StateEnum.无核酸检测结果)
                .reason("无核酸结果").build();
    }
    ...
    private StateResult getStrongEscortState(EscortMainInfo escortMainInfo) {
        ...
        List<FinIprInMainInfo> inMainInfos = inMainInfoMapper.queryInMainInfos(FinIprInMainInfoMapper.Key.builder()
                ...
                .happenNo(escortMainInfo.getHappenNo())
                .build());
    ...
    public StateResult getState(EscortMainInfo escortMainInfo) {
        ...
        if (states != null && !states.isEmpty()) {
            ...
            // 若已注销，则不操作
            if (states.get(0).getState() == StateEnum.注销) {
                return StateResult.builder()
                        .state(StateEnum.注销)
    ...
    private void canRegister(String patientCardNo, String helperCardNo, Boolean escape) {
        ...
        if (escortInfos != null && !escortInfos.isEmpty()) {
            ...
            if (!activeList.isEmpty()) {
                // 尚存有效陪护
                log.error(String.format("陪护关系已绑定(escortNo = %s)", json.toJson(activeList.stream().map((x) -> {
                ...
                if (escape == null || !escape) {
                    ...
                    if (duration.compareTo(guard) < 0) {
                        ...
                        String errStr = ageDurationToStringConverter.convert(offset);
    ...
    public SimulateResult simulateRegister(String patientCardNo, String helperCardNo, Boolean escape) {
        ...
        if (state.getState() == StateEnum.注销) {
